...
Fase 6 — Publicação (GitHub público)               [~] Em andamento (6.2 README e 6.3 LICENSE concluídos na Sessão 25, adiantados por pedido; 6.4 já estava satisfeita há tempos — o repo já tinha histórico público em `origin`/`gitea` bem antes desta sessão, checklist só nunca tinha sido marcado; 6.1 rodada na Sessão 47 — segredos/credenciais confirmados limpos contra o repo e o histórico completo do git, mas 2 achados de hardening/consistência (UI em português em vez de inglês, CSP desabilitado) ficaram registrados como pendência consciente, não corrigidos ainda por pedido do dono do projeto — ver Roadmap)
...
Fase 9 — Tela de Pesquisa (unificada, ex-"Stock Lookup")  [x] Completa — 9.1 a 9.5 concluídas; Sessão 43 (2026-08-02) generalizou a tela pra qualquer classe com dado automático (não só ação): renomeada "Pesquisa", ganhou seletor de classe explícito ao lado do ticker (Ação/FII), testada no app real
