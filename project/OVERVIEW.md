...
Fase 8 — Sync Multi-Dispositivo via TruthID+IPFS  [~] Em andamento — Sessão 23 (2026-07-16): transporte cross-device do `/sign-request` validado ponta a ponta; Sessão 31 (2026-07-28): 8.1 deployada de verdade em Base Sepolia (`0x9e2fd1a4146e3c40ecfef9d35a09e75e00ac30e8`) e 8.2 (escrita via canal delegado, same-machine) implementada e validada ponta a ponta pelo usuário no app real; Sessão 32 (2026-07-28): 8.3 (pinning via `/truthid/v1/pin`, pinando o SQLite real) implementada, pin real validado (Kubo local) — mas a leitura de confirmação (8.1) não trouxe o registro de volta, suspeita de rede errada (Base Mainnet vs Sepolia) ainda não confirmada, próxima sessão começa por aí antes de seguir pra 8.4/8.5; ver Fases Detalhadas
Fase 9 — Tela de Pesquisa de Ação (Stock Lookup)  [x] Completa — 9.1 a 9.5 concluídas, nenhuma pendência conhecida
Fase 10 — Remodelagem para Workspaces Colaborativos & Multi-Ativos [ ] Não iniciada — ideia grande registrada na Sessão 29 (workspaces/RBAC/multi-ativos/teses/watchlists + rentabilidade histórica via TWR); confirmado que fica na stack atual (Tauri+Rust+SQLite, sem servidor/Postgres/Django), carteira pertence ao Workspace (não a um membro), Workspace nasce single-user (convite/RBAC real fica bloqueado até resolver permissão descentralizada); ordem decidida pra quando começar: 10.1 fundação → 10.2 multi-ativos (primeira fatia real) → 10.3/10.4/10.5 (rentabilidade/watchlists/teses, ordem entre elas em aberto); Sessão 30 ampliou o escopo de multi-ativos com FII/REIT/ETF/Cripto/Metal/Imóvel/Empresa não listada + modelo de listagem/exposição/classe (item 8) e Gestão de Custódias/Contas (item 9) — ver Fases Detalhadas
